...
//				Profile.StartProfile("TestNewNorm");
				...
				if (renderType != MB_RenderType.skinnedMeshRenderer){ //for skinned meshes leave in bind pose
					for (int j = 0; j < nverts.Length; j++){
						nverts[j] = l2wMat.MultiplyPoint3x4(nverts[j]);
						if (_doNorm) {
							nnorms[j] = l2wRotScale.MultiplyPoint3x4(nnorms[j]);
							nnorms[j] = nnorms[j].normalized;
						}
						if (_doTan){
...
//				Profile.EndProfile("TestNewNorm");
				...
				if (_doUV1){
					nuv1s = MBVersion.GetMeshUV1s(mesh,LOG_LEVEL);
		...
		void _copyAndAdjustUVsFromMesh(MB_DynamicGameObject dgo, Mesh mesh, int vertsIdx){
			...
			if (textureBakeResults.fixOutOfBoundsUVs == false){
				...
				for (int i = 0 ; i < textureBakeResults.prefabUVRects.Length; i++){
					if (textureBakeResults.prefabUVRects[i] != ident){
						allAreIdent = false;
						break;
					}
				}
		...
		public override void Apply(bool triangles,
			...
			if (_mesh != null){
				if (LOG_LEVEL >= MB2_LogLevel.trace){
					Debug.Log(String.Format("Apply called tri={0} vert={1} norm={2} tan={3} uv={4} col={5} uv1={6} uv2={7} bone={8} meshID={9}",
						triangles, vertices,normals,tangents,uvs,colors,uv1,uv2,bones,_mesh.GetInstanceID()));	
				}
				...
				if (uv1){
					if (_doUV1) {
						MBVersion.MeshAssignUV1(_mesh,this.uv1s);
					}
		...
		void _updateGameObject(GameObject go, bool updateVertices, bool updateNormals, bool updateTangents, 
			...
			if (_doNorm && updateNormals){
				...
				for (int j = 0; j < nnorms.Length; j++){
					nnorms[j] = l2wMat.MultiplyPoint3x4(nnorms[j]);
					normals[dgo.vertIdx + j] = nnorms[j].normalized;					
				}
			}
			...
			if (_doUV1 && updateUV1){
				Vector2[] nuv1 = MBVersion.GetMeshUV1s(mesh,LOG_LEVEL);
...
//			Profile.StartProfile("AddDeleteGameObjectsByID");
			...
			//if (_outputOption == MB2_OutputOptions.bakeIntoSceneObject || 
			//   (_outputOption == MB2_OutputOptions.bakeIntoPrefab && _renderType == MB_RenderType.skinnedMeshRenderer) ||
			//	_outputOption == MB2_OutputOptions.bakeMeshAssetsInPlace){
...
//			Profile.PrintResults();
		...
		public void BuildSceneMeshObject(bool createNewChild=false){
			...
			_targetRenderer = BuildSceneHierarch(this, _resultSceneObject,GetMesh(),createNewChild);
		}		
